...
async function extractPageText() {
  try {
    ...
    if (
      ...
      addAIMessage(
        "현재 페이지는 브라우저 내부 페이지라서 내용을 읽을 수 없어요. 일반 웹페이지에서 다시 시도해주세요."
      );
      return;
    }
    ...
    if (error.message.includes("Receiving end does not exist")) {
      addAIMessage(
        "페이지가 아직 완전히 로드되지 않았어요. 페이지를 새로고침한 후 다시 시도해주세요."
      );
    } else if (error.message.includes("Cannot access")) {
      addAIMessage(
        "이 페이지는 보안상의 이유로 접근할 수 없어요. 다른 페이지에서 시도해주세요."
      );
    } else {
      addAIMessage(
        "페이지 내용을 읽어오는 중 문제가 발생했어요. 페이지를 새로고침한 후 다시 시도해주세요."
      );
    }
  }
}
...
async function callOpenAI(userMessage) {
  try {
    ...
    console.log(
      "시스템 프롬프트에 포함된 페이지 텍스트:",
      pageText.substring(0, 10000).length,
      "글자"
    );
    console.log("전체 메시지 배열:", JSON.stringify(messages, null, 2));
    console.log(
      "API 키 (마스킹):",
      apiKey
    ...
    if (!response.ok) {
      console.error("API 오류 발생 - 상태 코드:", response.status);
    ...
    console.log("응답 길이:", aiResponse.length, "글자");
